...
const names = ['tnuvaPaperCheck', 'tnuvaResetPhotoReceipt', 'tnuvaCachedDoc', 'tnuvaPhotoReady',
...
function context(extra = {}) {
  ...
  const c = vm.createContext({ console, setTimeout, clearTimeout, AbortController, Date, JSON, Math, Number,
    ...
    openReceivingScanner() {}, tnuvaEnsurePhotoService: async () => {}, aiOpenNextUnconfirmedOrientation() {}, aiReceivedProductIds: () => [],
...
const input = () => ({ noteIndex: 0, amount: null, units: null, lines: null, pages: [page()] });
const paper = (index = 0, amount = 50) => ({ noteIndex: index, pageCount: 1, subtotalExVat: amount, printedUnits: null, docType: 'invoice', itemsPrintedLines: 1,
  printedLines: 1, rows: [{ section: 'items', description: 'חלב', sourcePage: 1, quantity: 10, lineTotalExVat: amount }], confidence: 0.9 });
const payload = () => ({ ok: true, serviceVersion: 10, model: 'gpt-5.6-luna', requestId: 'fixture',
...
test('background completion refreshes the visible invoice header without replacing the active input', () => {
  ...
  vm.runInContext(['receiptNoteHeaderLabel', 'receiptNoteHeaderValue', 'refreshReceiptTotals', 'refreshScanHost', 'tnuvaPaperStatusHtml'].map(source).join('\n'), c);
  c.refreshScanHost();
  assert.equal(nodes.rcNoteSummaryValue.textContent, 'התעודה בפענוח…');
  c.aiScanDocuments = [input()];
  const doc = paper(0, 971.42); doc.rows[0].quantity = 244;
  c.aiScanResponse = { scan: { documents: [doc] } };
  assert.equal(c.tnuvaAdoptPaperAnchors(), true);
  assert.equal(nodes.rcNoteSummaryValue.textContent, '₪971.42');
  assert.match(nodes.rcNoteSummaryLabel.textContent, /1 שורות/);
...
test('cancel then new receipt ignores late OCR and never overwrites new counts', async () => {
  ...
  c.aiScanDocuments = [input()];
  c.aiRequestSingleDocScan = () => new Promise(done => { resolve = done; });
  const run = c.tnuvaStartPaperScan(); await tick();
  c.tnuvaResetPhotoReceipt();
...
test('first document survives second failure, reload and retry; only missing document is sent', async () => {
  ...
  assert.equal(restored.tnuvaCachedDoc(restored.aiScanDocuments[0]), true);
  assert.equal(restored.tnuvaCachedDoc(restored.aiScanDocuments[1]), false);
  restored.aiScanDocuments[1].pages = [page()];
  let secondCalls = 0; restored.aiRequestSingleDocScan = async () => { secondCalls++; return payload(); };
  await restored.tnuvaStartPaperScan();
  assert.equal(secondCalls, 1);
  assert.equal(restored.receiptNoteTotal, 100);
  assert.equal(restored.receiptNoteUnits, null);
  assert.equal(restored.receiptNoteLines, 2);
...
test('refresh before first result preserves document count and does not resubmit automatically', () => {
  ...
  assert.equal(restored.aiScanDocuments.length, 2);
  assert.equal(restored.tnuvaPhotoReady(), false);
...
test('each paper closes independently, even if two incorrect sums cancel overall', () => {
  ...
  assert.equal(c.tnuvaAdoptPaperAnchors(), false);
  assert.equal(c.receiptNoteTotal, null);
});
...
test('new image invalidates only that document cache', () => {
  ...
  c.tnuvaInvalidatePhotoDoc(b);
  assert.equal(c.tnuvaCachedDoc(a), true);
  assert.equal(c.tnuvaCachedDoc(b), false);
});
...
function comparisonContext(receivedId = 'milk') {
  const c = context({ promos: [], receipts: [], receiptDupConfirmed: true, receiptFix: null,
    ...
    AI_SCAN_MIN_SERVICE_VERSION: 1, AI_BARCODE_SUFFIX_DIGITS: 6, RECEIPT_ROUNDING_TOLERANCE_CENTS: 20,
  ...
  c.receiptNotes = [{ amount: 50, units: null, lines: 1 }]; c.recomputeNoteTotal();
  c.receiptOpened = true; c.receiptPaperScanState = 'ok'; c.receiptAnchorSource = 'paper';
  c.aiScanDocuments = [{ ...input(), amount: 50, units: null, lines: 1 }];
  const p = payload();
...
test('equal total/units still enters actual product comparison; equal-price swap is detected', () => {
  for (const productId of ['milk', 'coffee']) {
    ...
    c.aiCloseOrientationReview = () => {}; c.resetDetectiveQuestions = () => {}; c.aiRunAnalyzer = () => {};
...
test('old or unavailable scan service never receives a paid photo upload', async () => {
  const c = context(); vm.runInContext(source('tnuvaEnsurePhotoService'), c);
  c.aiScanDocuments = [input()];
  let calls = 0;
  c.fetch = async url => {
    calls++; assert.ok(url.endsWith('/health'));
    return { ok: true, json: async () => ({ ok: true, keyConfigured: true, serviceVersion: 9 }) };
  };
  await c.tnuvaStartPaperScan();
  assert.equal(calls, 1);
  assert.equal(c.receiptPaperScanState, 'failed');
...
test('actual response adapter keeps raw Tnuva paper and prevents mixed/credit adoption', async () => {
  for (const kind of ['invoice', 'mixed', 'credit']) {
    const raw = { ...paper(), docType: kind === 'credit' ? 'credit' : 'invoice', docNumber: 'TEST',
      promoDiscountExVat: 5, itemsSectionTotalExVat: 50, vatPct: 18,
      subtotalExVat: kind === 'mixed' ? 35 : kind === 'credit' ? -45 : 45,
      rows: [{ ...paper().rows[0], code: '8', unitPriceExVat: 5, promoStar: true }] };
    if (kind === 'mixed') {
      raw.returnsPrintedLines = 1; raw.returnsSectionTotalExVat = 10;
      raw.rows.push({ ...raw.rows[0], section: 'returns', quantity: 2, lineTotalExVat: 10 });
    }
    const original = JSON.stringify(raw);
    const c = context({ products: [{ id: 'milk', name: 'חלב', barcode: '7290000000008', price: 5 }],
    ...
    const adapted = response.scan.documents[0];
    assert.equal(JSON.stringify(adapted.__tnuvaPaper), original);
    assert.equal(adapted.documentDiscountExVat, 5);
    c.aiScanDocuments = [input()]; c.aiScanResponse = response;
    assert.equal(c.tnuvaAdoptPaperAnchors(), kind === 'invoice');
    if (kind === 'invoice') {
      assert.equal(c.receiptNoteTotal, 45); assert.equal(c.receiptNoteLines, 1); assert.equal(c.receiptNoteUnits, null);
      assert.equal(adapted.rows[0].barcode, '7290000000008');
    } else {
      assert.equal(c.receiptNoteTotal, null); assert.ok(c.receiptPaperScanProblems.length);
      assert.ok(adapted.rows.every(row => !row.section || row.section !== 'returns'));
    }
  }
});

test('repeated printed product codes use printed line count and aggregate real quantities at finishing', () => {
  const c = comparisonContext();
  const doc = c.aiScanResponse.scan.documents[0];
  doc.itemsPrintedLines = doc.printedLines = 2;
  const row = doc.rows[0];
  doc.rows = [{ ...row, quantity: 4, lineTotalExVat: 20, grossLineTotalExVat: 20 },
    { ...row, lineNumber: 2, quantity: 6, lineTotalExVat: 30, grossLineTotalExVat: 30 }];
  c.receiptNotes[0].lines = c.aiScanDocuments[0].lines = 2; c.recomputeNoteTotal();
  appFunctions(c, ['finishReceipt', 'openReconcile']);
  c.aiCloseOrientationReview = () => {}; c.resetDetectiveQuestions = () => {}; c.aiRunAnalyzer = () => {};
  c.showConfirm = () => { throw new Error('Printed duplicate lines must not be compared to unique scanned products'); };
  c.setView = view => { c.currentView = view; };
  c.finishReceipt();
  assert.equal(c.currentView, 'reconcile');
  assert.equal(c.aiScanEvaluation.totalVerified, true);
  assert.equal(c.aiScanEvaluation.unitsVerified, true);
